<template lang='html'>
  <div>
    ...
    <div class="selectArea">
      <span>学科：</span>
      <el-select v-model="selectSubjectId" placeholder="请选择" @change="subjectChange()">
        <el-option
      ...
      <span>年级：</span>
      <el-select v-model="selectGradeId" placeholder="请选择" @change="gradeChange()">
        <el-option
    ...
    <div class="btnArea">
      <el-button type="primary" @click="onlinePaper()">在线组卷</el-button>
          ...
          <template slot-scope="scope">
            ...
            >
              <ul class="popover-list">
                ...
                <li @click="downPaper(scope.row)">下载试卷</li>
                <li @click="downCard(scope.row)" v-if="false">下载答题卡</li>
                <!--<li @click="reusePaper(scope.row)">复用试卷</li>-->
  ...
  export default {
    ...
    methods: {
      ...
      async getScanExamPaperList() {
        let url = '/das/paper/manage/getScanExamPaperList'
        let params = {
          sid: this.global.sid,
          uid: this.global.uid,
      ...
      //在线组卷
      onlinePaper() {
        window.open(`${this.global.LOCAL_URL}/#/OnlinePaper/SelectItem`)
      },
      //试题分析
      itemAnalysis(row) {
        row.show = false
        window.open(`${this.global.LOCAL_URL}/#/SelfPaper/ItemAnalysis/${this.type}/${row.paperId}`)
      },
      //题目详情
      itemDetail(row) {
        row.show = false
        window.open(`${this.global.LOCAL_URL}/#/SelfPaper/ItemDetail/${this.type}/${row.paperId}`)
      },
      //试卷属性
      paperAttribute(row) {
        row.show = false
        window.open(`${this.global.LOCAL_URL}/#/SelfPaper/PaperAttribute/${this.type}/${row.paperId}`)
      },
      //下载答题卡
      downCard(row) {
        row.show = false
      ...
      //下载试卷
      downPaper(row) {
        window.open(`http://115.29.177.227:8080/dowload_paper-1.5.1.RELEASE/dowload/${this.global.uid}/${row.paperId}`)
        row.show = false
      },
      //复用试卷
      reusePaper(row) {
        row.show = false
        window.open(`${this.global.LOCAL_URL}/#/OnlinePaper/SetPaper/${row.paperId}`)
      },
      subjectChange() {
